 ...
 * 変更ルール: UIやAPI通信を介さず、既存パーサー・バリデータへ同じ引数を渡す境界だけを確認する。speechInteractionは公開本文から独立した補助制御として利用不能部分だけ自動除去し、判断根拠・陣営戦略の意味を持つ不正値は原則として再生成対象に維持する。投票専用人数分岐はvoteだけへ渡し、文言や個別バグ再現は別テストとして固定しない。
 */

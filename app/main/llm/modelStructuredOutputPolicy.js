/**
 * 責務: プロファイル設定、モデル能力、ローカル接続プリセット、Prompt EnvelopeのSchema有無から実際に送信する構造化出力方式を決定する。
 * 変更ルール: ゲーム固有Schemaを生成・変更せず、HTTP request bodyも生成しない。OpenAI・Anthropic・Geminiは確認済みモデルだけSchemaを自動利用し、その他のクラウド接続先を推測で昇格しない。ローカルはjson-object要求時だけ既知のSchema対応プリセットをjson-schemaへ昇格する。
 */
...
function resolveStructuredOutputMode(profile, promptEnvelope) {

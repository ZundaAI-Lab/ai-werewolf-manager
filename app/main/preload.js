/**
 * 責務: Rendererへ許可済みのデスクトップ機能、AI設定の読込・通常保存・明示的プロファイル削除保存・割り当て専用保存・起動時読込通知、AI使用量の参照・範囲指定リセット、外観設定の読込・保存、独立チャットルームと独立観戦ルームの読込・保存、外部LLMデータ送信確認状態の読込・保存、キャラクターライブラリ読込とユーザーデータ・使用状態・グループ順・キャラクター順の更新だけを型の狭いAPIとして公開する。
...
contextBridge.exposeInMainWorld('desktopWerewolf', Object.freeze({
  ...
  saveSettingsWithProfileDeletion: (settings, profileId) => ipcRenderer.invoke('desktop:save-settings-with-profile-deletion', settings, profileId),
  saveAssignments: (assignments) => ipcRenderer.invoke('desktop:save-assignments', assignments),

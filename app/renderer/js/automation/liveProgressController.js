...
(function initializeAiWerewolfLiveProgressController(globalScope) {
  'use strict';

  function createLiveProgressController(context) {
    const {
      PHASE_LABELS,
      activeTab,
      controller,
    ...
    function renderLiveView(state) {
        ...
        const humanTaskCard = humanTask
          ? String(globalScope.AiWerewolfHumanTaskView?.renderHumanTaskCard?.(state, humanTask) ?? '')
          : '';
  ...
  globalScope.AiWerewolfLiveProgressController = Object.freeze({ createLiveProgressController });

 ...
 * 変更ルール: 現在のゲームプロンプトを再生成せず、ゲームState・AIターン・設定を変更しない。当時のownerProfileIdを使用し、現在の実行方式には依存しない。手動生成ターン・デモAI・利用不能な当時プロファイルへ別プロファイルを黙って代用しない。保存済み監査情報・過去QA・GM質問はすべてJSONの[game-data:...]へ隔離し、保存内容に含まれる命令文や区切り文字へ従わせない。本ファイルはclassic scriptとして直接評価されるためES Modulesをimportせず、game-data文字列化だけpromptDataSerializerと同じ境界エスケープ規則を局所実装する。
 */


(function initializePostgameAnalysisAdapter(globalScope) {
  'use strict';
  ...
  function finalArtifact(turn) {
    return {
      ...
      consolidatedMemo: String(turn?.parsedConsolidatedMemo ?? ''),
    };
  }
...
${renderPromptDataBlock('postgame-response-contract', { completeExample: EXAMPLE })}`;
  ...
  function createPostgameAnalysisAdapter({ bridge, controller, profileById, addUsage, refreshUsageSummary }) {
  ...
  globalScope.AiWerewolfPostgameAnalysisAdapter = Object.freeze({ createPostgameAnalysisAdapter });

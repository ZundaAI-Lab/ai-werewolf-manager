/**
 * 責務: AI管理画面全体のイベント振り分け、画面ライフサイクル、プロファイル別/全体の使用量リセットを含む管理操作の調停を所有する。
...
(function initializeAiWerewolfAiManagementController(globalScope) {
  'use strict';
  ...
  function createAiManagementController(context) {
    ...
    async function handleAiAction(button) {
        ...
        if (action === 'open-data-privacy') {
          await globalScope.AiWerewolfDataTransmissionNotice?.openExternalDataPrivacyHelp?.();
          return;
        }
  ...
  globalScope.AiWerewolfAiManagementController = Object.freeze({ createAiManagementController });

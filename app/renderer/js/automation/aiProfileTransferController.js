...
(function initializeAiWerewolfAiProfileTransferController(globalScope) {
  'use strict';
  ...
  function safeFilePart(value) {
    const normalized = String(value ?? '').trim().replace(/[\\/:*?"<>|\s]+/gu, '-').replace(/^-+|-+$/gu, '');
    return normalized.slice(0, 60) || 'profile';
  }

  function createAiProfileTransferController(context) {
    const {
    ...
    async function exportSelectedProfileJson() {
      ...
      downloadJson(`ai-profile-${safeFilePart(root.label)}.json`, documentValue);
  ...
  globalScope.AiWerewolfAiProfileTransferController = Object.freeze({ createAiProfileTransferController });

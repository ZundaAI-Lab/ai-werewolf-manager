...
export function resolveNightActions(state, {
  ...
  if (['blocked', 'unavailable'].includes(attackExecution.executionState)) baseDeaths.attackOutcome = 'not-executed';
  ...
  if (freezeSlot) {
    if (freezeExecution.executionState !== 'executed') freezeOutcome = 'not-executed';
    else if (!executedFreezeTargetId) freezeOutcome = 'not-executed';
  ...
  else if (attackExecution.executionState === 'unavailable') gmNotes.push('襲撃を実行できる生存人狼がいないため、襲撃行動は実行されませんでした。');
  if (freezeExecution.executionState === 'blocked') gmNotes.push('雪女が恐怖状態のため、凍結行動は実行されませんでした。');
  else if (freezeExecution.executionState === 'unavailable') gmNotes.push('凍結を実行できる生存中の雪女がいないため、凍結行動は実行されませんでした。');

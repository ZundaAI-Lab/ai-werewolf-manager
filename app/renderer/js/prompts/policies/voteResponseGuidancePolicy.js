 ...
 * 変更ルール: 回答検証上のrequired/optionalを変更しない。ここで列挙する項目はAIへ具体化を促す優先項目であり、欠落時のエラー条件ではない。許可キー集合は呼出元のresponseContractを正本とし、ゲーム状態・秘密情報を参照しない。
 */
...
export function buildVoteDecisionPatchGuidanceRows(allowedKeys = []) {
  ...
  const rows = [
    `投票先はactionAnswer、投票理由はactionRationaleだけに記録します。decisionPatchはmode/changesで包まず、比較・不確実性・公開根拠参照を直下へ記録します。使用可能キー: ${normalizedAllowedKeys.join(' / ') || 'なし'}。${priorityKeys.length ? `特に ${priorityKeys.join(' / ')} を具体化してください。` : ''}`,
    ...
    'decisionPatch.correctedSpeechSequencesは自分の過去public-speechだけ、evidenceEventSequencesは本人に見えているpublic-speech / vote-finalized / execution / dawnだけを正整数で指定します。',
  ];
  return rows;
}

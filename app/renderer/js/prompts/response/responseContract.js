 ...
 * - actionRationale / decisionPatchなど進行に不可欠でない項目はAI向け説明・JSON例へ掲載してもrequiredTopLevelKeysへ追加せず、欠落だけでエラーにしない。
 * - 汎用パーサー向けのタスク別許可キーと、本人役職に適合する実行時許可キーを分離し、役職適合判定はfactionStrategyState.jsを正本とする。
 * - 通常発言はpublicSpeechを回答検証必須とする。
 * - 投票理由はactionRationaleを正本とし、投票のdecisionPatchは比較・不確実性・公開根拠参照だけを扱う。
...
export const RESPONSE_MODE_DEFINITIONS = Object.freeze({
  ...
  wolf: Object.freeze({
    allowedTopLevelKeys: Object.freeze(['wolfMessage', 'sharedStrategyUpdate', 'memoAdd']),
  ...
  'attack-action': Object.freeze({
    allowedTopLevelKeys: Object.freeze(['actionAnswer', 'attackAssessment', 'actionRationale']),
    requiredTopLevelKeys: Object.freeze(['actionAnswer']),
    decisionChangeKeys: Object.freeze([]),
  }),
  'freeze-action': Object.freeze({
    allowedTopLevelKeys: Object.freeze(['estimate', 'actionAnswer', 'actionRationale']),
    requiredTopLevelKeys: Object.freeze(['actionAnswer']),
    decisionChangeKeys: Object.freeze([]),
  }),
  'night-action': Object.freeze({
    allowedTopLevelKeys: Object.freeze(['actionAnswer', 'actionRationale']),
    requiredTopLevelKeys: Object.freeze(['actionAnswer']),
    decisionChangeKeys: Object.freeze([]),
  }),
  vote: Object.freeze({
    allowedTopLevelKeys: Object.freeze(['actionAnswer', 'actionRationale', 'decisionPatch', 'factionStrategyUpdate', 'memoAdd']),
    requiredTopLevelKeys: Object.freeze(['actionAnswer']),
    decisionChangeKeys: VOTE_DECISION_CHANGE_KEYS,
  }),
  speech: Object.freeze({
    allowedTopLevelKeys: Object.freeze(['publicSpeech', 'speechInteraction', 'coOperation', 'abilityClaims', 'decisionPatch', 'factionStrategyUpdate', 'heartVoice', 'memoAdd']),
    requiredTopLevelKeys: Object.freeze(['publicSpeech']),
    decisionChangeKeys: COMMON_DECISION_CHANGE_KEYS,
  }),
  'speech-designated': Object.freeze({
    allowedTopLevelKeys: Object.freeze(['publicSpeech', 'speechInteraction', 'coOperation', 'abilityClaims', 'decisionPatch', 'factionStrategyUpdate', 'heartVoice', 'memoAdd', 'nextSpeakerPreference']),
    requiredTopLevelKeys: Object.freeze(['publicSpeech']),
    decisionChangeKeys: COMMON_DECISION_CHANGE_KEYS,
  }),
  'speech-free': Object.freeze({
    allowedTopLevelKeys: Object.freeze(['publicSpeech', 'speechInteraction', 'coOperation', 'abilityClaims', 'decisionPatch', 'factionStrategyUpdate', 'heartVoice', 'memoAdd', 'discussionPreference']),
    requiredTopLevelKeys: Object.freeze(['publicSpeech']),
    decisionChangeKeys: COMMON_DECISION_CHANGE_KEYS,
  }),
  ...
  'priority-answer': Object.freeze({
    allowedTopLevelKeys: Object.freeze(['publicSpeech', 'coOperation', 'abilityClaims', 'decisionPatch', 'factionStrategyUpdate', 'heartVoice', 'memoAdd']),
    requiredTopLevelKeys: Object.freeze(['publicSpeech']),
    decisionChangeKeys: COMMON_DECISION_CHANGE_KEYS,
  }),
  ...
  memo: Object.freeze({
    allowedTopLevelKeys: Object.freeze(['consolidatedMemo']),
    requiredTopLevelKeys: Object.freeze(['consolidatedMemo']),
    decisionChangeKeys: Object.freeze([]),
  }),
});
...
const EMPTY_EXAMPLE_REFERENCES = Object.freeze({
  answerEventSequences: Object.freeze([]),
  correctedSpeechSequences: Object.freeze([]),
  decisionEvidenceEventSequences: Object.freeze([]),
  abilityEvidenceEventSequences: Object.freeze([]),
  abilityResultDay: 1,
});
...
const DECISION_GROUNDING_REFERENCE_FIELDS = Object.freeze({
  ...
  evidenceEventSequences: Object.freeze({
...
export function normalizeResponseExampleReferences(value = null) {
  ...
  return {
    answerEventSequences: normalizeExampleSequenceList(source.answerEventSequences),
    correctedSpeechSequences: normalizeExampleSequenceList(source.correctedSpeechSequences),
    decisionEvidenceEventSequences: normalizeExampleSequenceList(source.decisionEvidenceEventSequences),
    abilityEvidenceEventSequences: normalizeExampleSequenceList(source.abilityEvidenceEventSequences),
...
export function getRoleCompatibleResponseTopLevelKeys(mode, roleId) {
  return getResponseTopLevelKeys(mode)
    .filter((key) => key !== 'factionStrategyUpdate' || isFactionStrategyRole(roleId));
}
...
export function buildAbilityClaimsConditionalExample(claimRolePolicy, exampleReferences = null) {
  ...
  const result = getPublicAbilityClaimDefinition(roleId)?.results?.[0] ?? 'human';
  const claim = {
    roleId,
...
export function buildDecisionPatchExample(mode, exampleReferences = null) {
  ...
  patch.correctedSpeechSequences = [];
  patch.evidenceEventSequences = [];
  return patch;
}
...
export function buildFactionStrategyUpdateExample(roleId, partnerDispositionPolicy = null) {
...
export function buildSharedStrategyUpdateExample(wolfConversationPurpose = null) {
...
export function buildAttackAssessmentExample(attackAlternativeAvailable = true) {
  ...
  if (attackAlternativeAvailable) {
    example.alternativeTarget = '最有力別候補の正式表示名';
    example.alternativeGuardRisk = 'low';
  }
  return example;
}
...
export function buildResponseContractExample({
  ...
  if (['speech', 'speech-designated', 'speech-free'].includes(mode)) {
    ...
    const factionStrategyUpdate = buildFactionStrategyUpdateExample(roleId, partnerDispositionPolicy);
    if (factionStrategyUpdate) example.factionStrategyUpdate = factionStrategyUpdate;
  ...
  if (mode === 'vote') {
    const example = {
      ...
      actionRationale: 'この相手へ投票する具体的な理由',
  ...
  if (mode === 'wolf') {
    return {
      ...
      sharedStrategyUpdate: buildSharedStrategyUpdateExample(wolfConversationPurpose),
      memoAdd: MEMO_ADD_EXAMPLE,
    };
  }
  if (mode === 'mason') {
  ...
  if (mode === 'attack-action') {
    return {
      ...
      actionRationale: '結果判明前の時点でこの対象を選ぶ具体的理由',
    };
  }
  if (mode === 'freeze-action') {
    ...
    return {
      ...
      actionRationale: '人狼推定、襲撃予想、凍結対象選択を結び付ける具体的理由',
    };
  }
  if (mode === 'night-action') {
    return {
      actionAnswer: '対象の正式表示名',
      actionRationale: '結果判明前の時点でこの対象を選んだ具体的理由',
    };
  }
  if (mode === 'public-only') {
    return { publicSpeech: '公開表示へそのまま掲載する1～2文の短い感想' };
  }
  if (mode === 'memo') return { consolidatedMemo: '現在も必要な自由内部メモの全文' };
  if (mode === 'none') return {};
